...
public:
    ...
    // Quantize one sample given a pre-computed scale factor.
    // Returns an N-bit sign-magnitude code: MSB = sign, lower (N-1) bits = magnitude bin.
    size_t select_qntz_idx(double sample, double scale) const;
