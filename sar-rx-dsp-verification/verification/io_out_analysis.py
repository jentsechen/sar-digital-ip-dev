...
TXT_PATH = os.path.join(PROJ_ROOT,
                        ...
                        "sim_1", "behav", "xsim", "io_out_h_data_0_bin.txt")

N_BITS = 16    # bits per I or Q
N_CH = 4       # channels 0-3
FRAC_BITS = 12  # 1 sign + 3 integer + 12 fractional (Q3.12)


def to_signed16(bits):
    val = int(bits, 2)
    if val >= (1 << (N_BITS - 1)):
        val -= (1 << N_BITS)
...
if __name__ == "__main__":
    ...
    fig.update_layout(title="io_out_h_data_0 — IQ (16-bit integer)")

    out_html = os.path.join(os.path.dirname(__file__), "io_out_h_data_0.html")
